...
import { Sparkles, Compass, Lock, Unlock } from "lucide-react";
  ...
  return (
    <header id="app-header" className="sticky top-0 bg-white/90 backdrop-blur-md border-b border-gray-100 z-40 transition-all duration-300">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center">
        ...
        {/* Action Button */}
        <div className="flex-1 flex justify-end items-center gap-4 min-h-[44px]">
          {isEditorMode && (
            <button
              onClick={onOpenCreate}
              className="px-6 py-2.5 bg-black hover:bg-neutral-800 text-white rounded-full text-xs font-bold shadow-sm transition-all duration-200 flex items-center gap-2 group active:scale-95 animate-fade-in"
            >
          ...
          <button
            ...
            className={`p-2 rounded-full transition-all duration-200 cursor-pointer active:scale-95 ${
              isEditorMode 
